...
# --- Login and Initialization with Hardcoded Secrets ---
if not HF_TOKEN:
    print("Warning: HF_TOKEN is not set. Hugging Face login may fail.", flush=True)
else:
    login(token=HF_TOKEN)

openai_client = OpenAI(api_key=OPENAI_API_KEY)
if not OPENAI_API_KEY:
    print("Warning: OPENAI_API_KEY is not set. Flawed CoT generation will fail.", flush=True)

if WANDB_API_KEY:
    print(f"Logging into WandB...", flush=True)
    wandb.login(key=WANDB_API_KEY)
else:
    print("Warning: WANDB_API_KEY not set. Run logging may fail or require manual login.", flush=True)
...
def binary_reward_function(prompts, completions, correct_answer, problem,
    ...
    with open("all_rewards_1.csv","a") as f:
           for r in rewards:
               f.write(f"{r}\n")
    return rewards
...
try:
    ...
    import traceback
    traceback.print_exc()
    # Re-raise to ensure an error status if it's a critical error
    raise
finally:
    print("\n" + "="*70, flush=True)
...
# Re-define extract_answer and normalize_answer for this section
...
try:
    with open("perturbed100.json", "r") as f:
        perturbed_dataset = json.load(f)
    print(f"Loaded {len(perturbed_dataset)} examples from perturbed100.json", flush=True)
except FileNotFoundError:
    print("Error: perturbed100.json not found in the current directory.", flush=True)
    perturbed_dataset = [] # Empty list to avoid further errors

correct_perturbed = 0
total_perturbed = 0
limit_perturbed = 100

# Use policy_model from training phase
for i, example in enumerate(tqdm(perturbed_dataset[:limit_perturbed])):
    if i >= limit_perturbed:
        break
    ...
    if i < 5: # Print first 5 for debugging
        ...
        print(f"Ground Truth: {gt_answer}", flush=True)
        print(f"Predicted: {pred_answer}", flush=True)
        print(f"Correct: {is_correct}", flush=True)

    if (i + 1) % 50 == 0:
        print(f"\nProgress: {i+1}/{limit_perturbed} - Accuracy: {correct_perturbed/total_perturbed:.2%}", flush=True)

# Final results for perturbed dataset
print(f"\n{'='*80}", flush=True)
print(f"Final Results on perturbed100.json:", flush=True)
print(f"Correct: {correct_perturbed}/{total_perturbed}", flush=True)
print(f"Accuracy: {correct_perturbed/total_perturbed:.2%}", flush=True)
print(f"{'='*80}", flush=True)

# --- Final message confirming all_rewards_1.csv saved ---
print("\n'all_rewards_1.csv' has been saved with training rewards.", flush=True)
