...
from tqdm import tqdm # for evaluation loop
...
class FlawedCoTGenerator:
    ...
    def generate_flawed_cot(self, problem: str, correct_answer: str, debug: bool = False) -> str:
        ...
        1.Introduce exactly one reasoning error. Here are examples:
Treating an implication as a biconditional (necessary vs. sufficient confusion)
Ignoring a valid case (incomplete casework, missing roots or branches)
Assuming independence of events without justification
Double-counting or undercounting in combinatorics
Overgeneralizing from a few small cases (pattern ⇒ proof)
Assuming linear/“nice” behavior where it’s not guaranteed
Using unjustified symmetry / invalid “WLOG”
...
def binary_reward_function(prompts, completions, correct_answer, problem,
    ...
    with open("all_rewards_2.csv","a") as f:
           for r in rewards:
               f.write(f"{r}\n")
    return rewards
...
print("Loading MATH dataset from JSON (perturbed100.json)...")
try:
...
# Redefine extract_answer if it was not consistent (already defined above, just ensuring its presence)
...
# Redefine normalize_answer if it was not consistent (already defined above, just ensuring its presence)
...
correct_eval = 0
total_eval = 0
limit_eval = 100 # Evaluate up to 100 examples from the perturbed dataset

for i, example in enumerate(tqdm(eval_dataset[:limit_eval], desc="Evaluating on perturbed100.json")):
    if i >= limit_eval:
        break

    question = example.get('prompt_with_flawed_start', example.get('problem', ''))
    ground_truth = example.get('correct_answer', example.get('solution', ''))

    if not question or not ground_truth:
        print(f"Skipping example {i} due to missing problem or correct answer.")
        continue
    ...
    if (i + 1) % 50 == 0:
        print(f"\nProgress on perturbed100.json: {i+1}/{limit_eval} - Accuracy: {correct_eval/total_eval:.2%}")
